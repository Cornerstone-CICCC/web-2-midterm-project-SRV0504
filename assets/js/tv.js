...
function setupModalListeners() {
  document.body.addEventListener("click", async (e) => {
    ...
    try {
      const res = await fetch(withKey(`/tv/${movieId}`));
...
async function loadCarousel() {
  try {
    const res = await fetch(withKey("/trending/tv/day"));
    const data = await res.json();
...
async function loadTopRated() {
  try {
    const res = await fetch(withKey("/tv/top_rated?page=1"));
    const data = await res.json();
...
async function loadNewReleases() {
  try {
    const res = await fetch(withKey("/tv/on_the_air?page=1"));
    const data = await res.json();
...
async function loadNowPlaying() {
  try {
    const res = await fetch(withKey("/tv/airing_today?page=1"));
    const data = await res.json();
...
async function loadHexGrid() {
  const container = $id("hexGrid");
  if (!container) return;

  try {
    const res = await fetch(withKey("/movie/popular?page=1"));
    const data = await res.json();
    const movies = (data.results || []).slice(0, 15);

    container.innerHTML = movies.map(m => `
      <div class="hex card" data-id="${m.id}">
